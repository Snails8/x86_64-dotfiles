    ...
    context.subscriptions.push(vscode.debug.registerDebugConfigurationProvider('php', {
        async resolveDebugConfiguration(folder, debugConfiguration, token) {
            ...
            if (debugConfiguration.program && !debugConfiguration.runtimeExecutable) {
                ...
                const executablePath = conf.get('executablePath') || conf.get('validate.executablePath');
                if (executablePath) {
                ...
                if (!debugConfiguration.runtimeExecutable) {
                    try {
                        ...
                        const selected = await vscode.window.showErrorMessage('PHP executable not found. Install PHP and add it to your PATH or set the php.executablePath setting', 'Open settings');
